...
class AttachmentsAdminController extends JController
{
	...
	function listAttachments()
	{
		...
		// Assemble the query
		$query = "SELECT a.*, a.id as id, u.name as uploader_name FROM #__attachments as a "
		...
		// Process each attachment and useful info
		foreach ($rows as $row) {
		...
		$view->assignRef('attachments', $rows);
	...
	function add()
	{
	   ...
	   function jSelectArticle(id, title) {
		...
		if ( $uri_type == 'file' ) {
			...
			$upload_toggle_url = 'index.php?option=com_attachments&amp;task=add';
			}
	...
	function edit()
	{
		...
		// Massage the data
		$attachment->size = intval( 10 * $attachment->file_size / 1024.0 ) / 10.0;
	   ...
	   function jSelectArticle(id, title) {
		...
		$change_parent_url = "index.php?option=com_attachments&amp;task=edit&amp;cid[]=$attachment_id&amp;change=parent";
	...
	function save()
	{
		...
		if ( $parent_changed AND ( intval($row->parent_id) == -1 ) ) {
		...
		if ( $parent_changed ) {
			...
			if ( ($new_uri_type == 'url') AND ($old_uri_type == 'file') ) {
				...
				$db =& JFactory::getDBO();
				$query = "SELECT filename_sys, id FROM #__attachments WHERE id='".intval($row->id)."' LIMIT 1";
				$db->setQuery($query);
	...
	function saveNew()
	{
		...
		switch ($this->_task) {
			case 'applyNew':
				$link = 'index.php?option=' . $option . '&task=edit&cid[]=' . intval($row->id);
				break;
	...
	function _saveParams()
	{
		...
		$new_secure = intVal($new_params->get('secure'));
	...
	function adminUtils()
	{
		...
		$url_top = JURI::base() . "index.php?option=com_attachments&amp;controller=special";
	...
	function warning()
	{
		...
		$lang =& JFactory::getLanguage();
		if ( $lang->isRTL() ) {
			$document->addStyleSheet( $mainframe->getSiteURL() .
									  'plugins/content/attachments_rtl.css',
									  'text/css', null, array() );
			}
