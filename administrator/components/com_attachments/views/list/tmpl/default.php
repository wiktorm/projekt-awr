...
$pageNav = new JPagination(count($this->attachments), $this->limitstart, $this->limit);
...
<div class="attachments_filter">
 ...
 for ($i=0, $n=count( $this->attachments ); $i < $n; $i++) {
	 ...
	 if ( !$row->parent_published AND $list_for_parents == 'published' ) {
		 continue;
		 }
	 if ( $row->parent_published AND $list_for_parents == 'unpublished' ) {
		 continue;
		 }
	 ...
	 $size = intval(10 * $row->file_size / 1024) / 10.0;
	 ...
	 if ($row->parent_id != $last_parent_id
	  OR $row->parent_type != $last_parent_type
