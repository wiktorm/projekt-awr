...
class AttachmentsViewUpdate extends JView
{
	...
	function display($tpl=null, $error=false, $error_msg=false)
	{
		...
		AttachmentsHelper::addStyleSheet( JURI::root() . 'plugins/content/attachments.css', $echo_css );
		AttachmentsHelper::addStyleSheet( JURI::root() . 'plugins/content/attachments2.css', $echo_css );

		// Handle RTL styling
		if ( $lang->isRTL() ) {
			AttachmentsHelper::addStyleSheet( JURI::root() . 'plugins/content/attachments_rtl.css', $echo_css );
			}
...
<?php endif; ?>
		...
		if ( $update == 'file' OR $uri_type == 'file' ) {
			...
			$controller->display($attachment->parent_id, $attachment->parent_type, $this->parent_entity,
