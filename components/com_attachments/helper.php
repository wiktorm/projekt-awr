...
class AttachmentsHelper
{
	...
	function addStyleSheet($css_path, $echo=false)
	{
		...
		elseif ( $caching ) {
			...
			if ( !$added_js ) {
				$js_path = JURI::root() . 'plugins/content/attachments_caching.js';
	...
	function add_view_urls(&$view, $save_type, $parent_id, $parent_type, $attachment_id, $from)
	{
		...
		$save_url = $url_base . "&task=" . $save_task . $component;
		if ( $from == 'closeme') {
		...
		if ( $save_type == 'upload' ) {
			$upload_file_url = $url_base . "&task=$upload_task" . $parentinfo . $component;
			$upload_url_url	 = $url_base . "&task=$upload_task&uri=url" . $parentinfo . $component;
		...
		elseif ( $save_type == 'update' ) {
			...
			$change_file_url =	$change_url . "&amp;update=file" . $component;
			$change_url_url =  $change_url . "&amp;update=url" . $component;
			$normal_update_url =  $change_url . $component;
	...
	function upload_file(&$row, &$parent, $attachment_id=false, $save_type='update')
	{
		...
		if ( !empty($new_attachment_id) ) {
			$attachment_id = intval($new_attachment_id);
			}
					 ...
					 $update=false, $attachment_id=false)
	{
		...
		if ( $old_filename_sys ) {
			...
			if ( JFile::exists($old_filename_sys) ) {
				echo JFile::delete($old_filename_sys);
				AttachmentsHelper::clean_directory($old_filename_sys);
				}
			}
	 ...
	 */
	function attachmentListHTML($parent_id, $parent_entity, $user_can_add, $Itemid, $from,
